  ...
  function Settings() {
    ...
    const editarPrecio = (e) =>{
        e.preventDefault();
        let id = e.target.id
        let precio = document.getElementById(`p_${id}`).value
        if (precio != ""){
          axios.put(`http://localhost:8000/api/settings/${id}`, { value: precio })
            .then(res => {
              setPosts(posts.map(p => p.id === id ? { ...p, value: precio } : p));
            })
            .catch(err => console.log(err));
        } else {
          alert("Per Favor no deixeu l'input en blanc")
        }
                ...
                {posts.map(post => (
                  <tr key={post.id}>
                    ...
                    <td className="acction_btn">
                        <form onSubmit={editarPrecio} id={post.id}>
